...
- (void)viewDidLoad {
  ...
  [self.view addSubview:self.videoCallBtn];
    ...
    // 添加发送名片请求按钮
    self.cardRequestBtn = [[UIButton alloc]init];
    self.cardRequestBtn.backgroundColor = [UIColor whiteColor];
    [self.cardRequestBtn setTitle:@"发送名片" forState:UIControlStateNormal];
    [self.cardRequestBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    // action
    [self.cardRequestBtn addTarget:self
                          action:@selector(btnRequestToCard:)
                forControlEvents:UIControlEventTouchUpInside];
    self.cardRequestBtn.translatesAutoresizingMaskIntoConstraints = NO;
    [self.view addSubview:self.cardRequestBtn];
  ...
  self.cardRequestBtn.layer.masksToBounds = YES;
  self.cardRequestBtn.layer.cornerRadius = 5.f;
}
...
// 发送名片
- (void)btnRequestToCard:(id)sender {
    // TODO: API请求发送名片
    // 发送过去  服务器判断该名片是否已经发送过，返回一个标记值
    [self.cardRequestBtn setTitle:@"已发送名片" forState:UIControlStateNormal];
    self.cardRequestBtn.enabled = false;
}
...
{  // 语音
  if ([[RCCallClient sharedRCCallClient] isAudioCallEnabled:ConversationType_PRIVATE]) {
    ...
    self.cardRequestBtn.hidden = YES;
  }
  // 视频
  ...
  self.subViews = NSDictionaryOfVariableBindings(_infoView,_conversationBtn,remarksView,_audioCallBtn,_videoCallBtn,_cardRequestBtn);
...
// card
    [self.view addConstraints:[NSLayoutConstraint
                               constraintsWithVisualFormat:@"V:[_cardRequestBtn(43)]"
                               options:0
                               metrics:nil
                               views:self.subViews]];
  ...
  [self.view addConstraints:[NSLayoutConstraint
    ...
    // card
    [self.view addConstraints:[NSLayoutConstraint
                               constraintsWithVisualFormat:@"H:|-10-[_cardRequestBtn]-10-|"
                               options:0
                               metrics:nil
                               views:self.subViews]];
  ...
  [self.view addConstraints:[NSLayoutConstraint
                             constraintsWithVisualFormat:@"V:[_conversationBtn]-15-[_audioCallBtn]-15-[_videoCallBtn]-15-[_cardRequestBtn]"
                             options:0
                             metrics:nil
                             views:self.subViews]];
